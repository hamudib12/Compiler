...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_and_clauses expr = function
    | [] -> expr
    | expr' :: exprs ->
            let cdr_sexpr = macro_expand_and_clauses expr' exprs in
            ScmPair (ScmSymbol "if", ScmPair (expr, ScmPair (cdr_sexpr, ScmPair (ScmBoolean false, ScmNil))))
    (* | _ -> raise (X_syntax "invalid and-expression");; *)
  ...
  let rec tag_parse sexpr =
    ...
    (match (scheme_list_to_ocaml ribs) with
      |ribs_list,ScmNil ->
      let vars_list = List.map (fun x-> match x with |(ScmPair(variable, ScmPair(value, ScmNil))) -> variable | _ -> raise (X_syntax "no match arguments")) ribs_list in
      let values_list = List.map (fun x-> match x with |(ScmPair(variable, ScmPair(value, ScmNil))) -> value | _ -> raise (X_syntax "no match arguments")) ribs_list in
      tag_parse (ScmPair(ScmPair(ScmSymbol("lambda"), ScmPair(Reader.scheme_sexpr_list_of_sexpr_list vars_list, exprs)), Reader.scheme_sexpr_list_of_sexpr_list values_list))
      |_ -> raise (X_syntax "invalid ribs list"))
    ...
    | ScmPair (ScmSymbol "let*",
               ScmPair (ScmPair (ScmPair (var,
                                          ScmPair (arg, ScmNil)),
                                 ribs),
                        exprs)) -> tag_parse (ScmPair (ScmSymbol "let",ScmPair
                                                                            (ScmPair
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       ...
       | ribs_list, ScmNil ->
         ...
         let old_arguments = List.map (fun x-> match x with |(ScmPair(var, ScmPair(value, ScmNil))) -> (var, value) | _ -> raise (X_syntax "no match arguments")) ribs_list in
         let new_ribs = List.map (fun (var, _) -> ScmPair(var, ScmPair(whatever, ScmNil))) old_arguments  in
         let new_body = List.fold_right
              (fun (var, old_args) so_far ->
                     ScmPair(ScmPair(ScmSymbol("set!"), ScmPair(var, ScmPair(old_args, ScmNil))), so_far)) old_arguments exprs in
         tag_parse(ScmPair(ScmSymbol("let"), ScmPair(Reader.scheme_sexpr_list_of_sexpr_list new_ribs, new_body)))
       | _ -> raise (X_syntax "invalid ribs list"))
  ...
  let rec sexpr_of_expr = function
    ...
    | ScmOr(exprs) ->
       ScmPair (ScmSymbol "or",
                (Reader.scheme_sexpr_list_of_sexpr_list (List.map sexpr_of_expr exprs)))
    | ScmSeq([]) -> ScmVoid
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let cross_product as' bs' =
   ...
   let get_rib name env =
      try List.find (fun rib -> List.mem name rib) env
      with Not_found -> raise (X_this_should_not_happen "")
   ...
   let should_box_var name expr params = 
    ...
    if reads = [] then false
      ...
      if writes = [] then false
        ...
        let readsXwrites = cross_product reads writes in
        let rec run = function
        | [] -> false
        | ((Var' (_, Param _), _), (Var' (_, Param _), _)) :: exp -> run exp
        | ((Var' (_, Bound _), env1), (Var' (_, Bound _), env2)) :: exp -> (get_rib name env1 != get_rib name env2) || (run exp)
        | ((Var' (_, Param _), _), (Var' (_, Bound _), _)) :: exp -> true
        | ((Var' (_, Bound _), _), (Var' (_, Param _), _)) :: exp -> true
        | _ :: exp -> run exp in
        run readsXwrites;;
