...
(*create a list from string chars*)
...
(*create a string from list's chars*)
...
(*check if the predicate f works on at least one element of the list*)
let rec ormap f s =
  match s with
  | [] -> false
  | car :: cdr -> (f car) || (ormap f cdr);;

(*check if the predicate f works on all the element of the list*)
let rec andmap f s =
  match s with
  | [] -> true
...
(*I wrote all of these functions on my notebook*)
